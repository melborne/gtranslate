...
class TestParseSite < Test::Unit::TestCase
  def setup
    ...
    @site = ParseSite.new(blog)
  end
  
  def test_get
    assert_equal(10, @site.get(:range => 1..10).size)
  end
  
  def test_check_url_error
    no_url = "htt://d.hatena.ne.jp/"
    assert_raise(ArgumentError) { @site.get(:url => no_url) }
  end
  
  def test_open_uri_error
    bad_url = "http://d.hatena.ne.jp/30000101/p1"
    assert_raise(OpenURI::HTTPError) { @site.get(:url => bad_url) }
  end
